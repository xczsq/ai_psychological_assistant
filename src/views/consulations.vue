<template>
  <div>
     <page-head title="咨询记录"></page-head>
           ...
           <div class="session-detail">
               <div class="detail-header">
                  <div class="detail-row">
                      ...
                      <div class="detail-value">{{ sessionDetail.userNickname || 'N/A' }}</div>
                  </div>
                  <div class="detail-row">
                      <div class="detail-label">开始时间</div>
                      <div class="detail-value">{{ sessionDetail.startedAt || 'N/A' }}</div>
                  </div>
                  <div class="detail-row">
                      <div class="detail-label">消息数量</div>
                      <div class="detail-value">{{ sessionDetail.messageCount || 'N/A' }}</div>
                  </div>
               </div>
                   ...
                   <div class="messages-list" v-loading="loadingMessages"> 
                        <div v-for="message in sessionMessages" :key="message.id" class="message-item" :class="message.senderType === 1 ? 'user-message' : 'ai-message'">
                             <div class="message-header">
                                 ...
                                 <span class="time">{{ message.createdAt || 'N/A' }}</span>
                             </div>
                             <div class="message-content">{{ message.content || 'N/A' }}</div>
                        </div>
                   </div>
               </div>
           </div>
...
import { getConsulationsPage,getSessionDetail } from '@/api/admin'
...
const handleSearch=()=>{
    getConsulationsPage(pagination).then(res=>{
